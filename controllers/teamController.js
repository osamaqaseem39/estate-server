...
/** Accept dashboard `image` or legacy `imageUrl`. */
function resolveImageUrl(body) {
  if (!body) return '';
  if (body.imageUrl != null && String(body.imageUrl).trim() !== '') return String(body.imageUrl).trim();
...
exports.createTeamMember = async (req, res) => {
  try {
    const body = req.body;
    let imageUrl = resolveImageUrl(body);
    if (req.file) {
    ...
    await doc.save();
    res.status(201).json(toPublicTeam(doc));
  } catch (err) {
...
exports.updateTeamMember = async (req, res) => {
  try {
    ...
    } else if (body.imageUrl != null || body.image != null) {
      update.imageUrl = resolveImageUrl(body);
    }
...
exports.listPublishedTeam = async (req, res) => {
  try {
    ...
    res.json(items.map(toPublicTeam));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
